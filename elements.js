// Elements module for selecting DOM elements
export const elements = {
    // Planets and Sun
    mercury: document.querySelector(".mercury"),
    venus: document.querySelector(".venus"),
    earth: document.querySelector(".earth"),
    mars: document.querySelector(".mars"),
    jupiter: document.querySelector(".jupiter"),
    saturn: document.querySelector(".saturn"),
    uranus: document.querySelector(".uranus"),
    neptune: document.querySelector(".neptune"),
    sun: document.querySelector(".sun"),

    // Earth-related sections
    earth1: document.querySelector('.earth1'),
    earth2: document.querySelector('.earth2'),
    earth3: document.querySelector('.earth3'),

    // Page and background sections
    infoPage: document.querySelector('.info-page'),
    background: document.querySelector('.background'),
    startPage: document.querySelector('.start-page'),

    // Info display sections for planet details
    name: document.querySelector('.name'),
    latinName: document.querySelector('.latinName'),
    artInfo: document.querySelector('.artInfo'),
    circumference: document.querySelector('.circumference'),
    fromSun: document.querySelector('.fromSun'),
    maxTemp: document.querySelector('.maxTemp'),
    minTemp: document.querySelector('.minTemp'),

    // Moons info
    moons: document.querySelector('.moons'),
};